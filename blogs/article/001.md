...
<p>在函数内部，我们首先通过 var 关键字声明了 name 变量。这意味着变量被提升了（内存空间在创建阶段就被设置好了），直到程序运行到定义变量位置之前默认值都是 undefined。因为当我们打印 name 变量时还没有执行到定义变量的位置，因此变量的值保持为 undefined
</p>
<p>
...
<p>
一元操作符加号尝试将 boolean 转为 number。true 转换为 number 的话为 1，false 为 0
</p>
<p>
...
<img :src="$withBase('/article/001/1.png')" />
<p>
...
<img :src="$withBase('/article/001/2.png')" />
</details>
...
+ A: `21`
+ B: `undefined`
+ C: `ReferenceError` // 当对变量/项的引用被破坏或不存在时，将引发此错误
+ D: `TypeError` // 指对象用来表示值的类型非预期类型时发生的错误
...
<img :src="$withBase('/article/001/4.png')" />
<p>
现在，foo 被调用，打印 "First"
</p>
<img :src="$withBase('/article/001/5.png')" />
<p>
foo 从栈中弹出，baz 被调用。打印 "Third"
</p>
<img :src="$withBase('/article/001/6.png')" />
<p>
WebAPI 不能随时向栈内添加内容。相反，它将回调函数推到名为 queue 的地方
</p>
<img :src="$withBase('/article/001/7.png')" />
<p>
这就是事件循环开始工作的地方。一个事件循环查看栈和任务队列。如果栈是空的，它接受队列上的第一个元素并将其推入栈
</p>
<img :src="$withBase('/article/001/8.png')" />
<p>
...
<p>
只有 8 种 falsy 值：undefined、null、NaN、false、''(empty string)、0、-0、0n (BigInt(0))
</p>
<p>
...
<p>
取决于你的运行环境（每个浏览器，以及 node 环境，都有可能不同）
</p>
</details>
...
<img :src="$withBase('/article/001/12.png')" />
<p>
...
<img :src="$withBase('/article/001/13.png')" />
<p>
...
console.log(person);
conosle.log(Object.keys(person));
```
...
const num2 = increatePassedNumber(num1);
...
<a href="https://zh.wikipedia.org/wiki/%E5%89%AF%E4%BD%9C%E7%94%A8_(%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%A7%91%E5%AD%A6)" title="纯函数与副作用的定义">
纯函数与副作用的定义
</a>
</details>
...
<p>
通过for-in循环，我们可以遍历一个对象自有的、继承的、可枚举的、非 Symbol 的属性。 在数组中，可枚举属性是数组元素的 “键”， 即它们的索引。 类似于下面这个对象：
</p>
<code>
{0: "☕", 1: "💻", 2: "🍷", 3: "🍫"}
</code>
...
conosle.log(giveLydiaPizza.prototype);
